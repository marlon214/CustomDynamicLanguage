/*
Name: Marlon Alejandro, 5002573038, CS326-1002, ASSIGNMENT_3
Description: Implementing static and dynamic scope
Input: user inputted commands on terminal
Output: execution on terminal
*/
...
//import "/Users/marlonalejandro/Downloads/Assn3/code/Lexer.java"
...
//                System.out.println(value);
                // returns the value our variable is bound too
...
//          LT, GT, LE, GE, EQ, NE, AND, OR, NOT
            ...
            // compares the left and right value
            switch (op) {
                case LT:
                    return new Value(l < r);
                case GT:
...
//                 CODE HERE
                // not working??
//                debug:System.out.println(value.eval());
                returnVariables.push(value.eval( ));
...
//         statement representing the sequence.
    ...
    public static class Block extends Stmt {
        ...
        public void exec() {
            // TODO: students need to complete this
            /*f*/
//            System.out.println("why is no one helping me");
            table.openScope();
            body.exec();
            table.closeScope();
...
//                System.out.println("I'm dying");
                if (condition.eval().getBoolean()&&ifBlock!=null) {
                    ifBlock.exec();
...
//                getNext().exec();
    ...
    public static class Lambda extends Expr {
        ...
        protected Value eval() {
            ...
            if (!error) {
//                return new Value(//lamda)
                // CODE HERE
                // initializes our new funtion
        ...
        protected Value eval() {
            ...
            if (!error) {
                ...
                if (funExpr.eval().getType() == Type.FUN_T) {
                    // creates the scope for our function
//                    table.openScope();
//                    table.bind(lambda.getVariable(), arg.eval());

                    // Execute the body of our function
//                    ReturnExpr lambda.getBody().exec();
                    // Debug:System.out.println("GodPleaseHelpMe");
                    Lambda lambda = funExpr.eval().getLambda();
                    // intializes our scope for our function
                    table.openScope();
                    table.bind(lambda.getVariable(), arg.eval());
                    lambda.getBody().exec();
                    //debug: System.out.println("dont break now!");
                    //debug: System.out.println(returnVariables.isEmpty());
                    // retreives the value we pusehd in our return expression
//                  //debug: System.out.println(returnVariables.peek());

                    // if no return value was given, return empty value
                    if(returnVariables.isEmpty()){
                        table.closeScope();
                        return new Value();
                    }
                    // retreives the value we pusehd in our return expression
                    Value returnValue = returnVariables.pop();
                    table.closeScope();
                    return returnValue;
                    // Retrieve the return value from the lambda function
//                    Value returnValue = returnVariables.peek();
                    // Debug:System.out.println(returnValue);
                    // Close the scope created for the function call
//                    table.closeScope();
...
//            System.out.println(val);
    ...
    public static class SymbolTable {
        ...
        Value lookup(final String name) {
            ...
            //looks up the value thats is bound to name
            Stack<Value> val = binding.get(name);
            // Debug:System.out.println("lookup");
            // Debug: System.out.println(returnValue.peek());
        ...
        public void bind(final String name, Value val) {
            // TODO: write the implementation
            //looks at the variables of our current scope and checks if our new variable exists
            if(!binding.containsKey(name)) {
                binding.put(name, new Stack<Value>());
                ...
                //Debug: System.out.println("Scope level: "+scopeVariables.size());
                //Debug: System.out.println("Binding '"+ val +"' to '"+ name +"'");
                binding.get(name).push(val);

                //if it doesnt, well add it to our local variabeles
                scopeVariables.peek().add(name);
//              scopeVariables.peek();

                // we would then add it to our map and assign its value
//                binding.put(name, new Stack<Value>());
//                binding.get(name).push(val);
                // Debug: System.out.println(binding.get(name).peek().getType());
//
            } else {
                error = true;
        ...
        void rebind(final String name, Value val) {
            // TODO: write the implementation
            if (binding.containsKey(name)) {
                // Debug:System.out.println("help");
                // finds out variable and deletes the existing value bound to it
                binding.get(name).pop();
                // rebinds the variable with new value
                binding.get(name).push(val);
            } else {
                error = true;
        ...
        public void openScope() {
            ...
            // CODE HERE
            // initializes  String set and pushes it to our stack
            // a set doesn't allow duplicate values
            Set<String> stringSet = new HashSet<>();
            scopeVariables.push(stringSet);
        }
        ...
        public void closeScope() {
            ...
            // CODE HERE
            // checks if any variables were initialized within our scope
            if (!scopeVariables.isEmpty()) {
                Set<String> stringSet = scopeVariables.pop();

                for (String variable : stringSet) {
                    Stack<Value> valueStack = binding.get(variable);

                    if (valueStack != null && !valueStack.isEmpty()) {
                        // Pop the value associated with the variable
                        valueStack.pop();

                        if (valueStack.isEmpty()) {
                            // If there are no more values for the variable, remove it from the binding
                            binding.remove(variable);
                            System.out.println("INFO: Removing '"+variable+"' from scope.");
                        }
